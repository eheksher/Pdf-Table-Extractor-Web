    ...
    function populateFilterMenu(columnName) {
      ...
      html += '<button onclick="applyFilter(\'' + columnName + '\')">החל סינון</button>';
      html += '</div>';
    ...
    function deleteTemplate() {
      ...
      if (!templateName) {
        showToast('אנא בחר תבנית למחיקה');
        return;
      }
...
function applyFilter(columnName) {
  const menu = document.getElementById('filter-' + columnName);
  const checkboxes = menu.querySelectorAll('.filter-option input[type="checkbox"]');
  columnFilters[columnName] = [];

  checkboxes.forEach(cb => {
    if (cb.checked) {
      const label = cb.parentNode.textContent.trim();
      columnFilters[columnName].push(label);
    }
  });

  renderTable(); // מפעיל את הסינון בפועל
  menu.classList.remove('active'); // סוגר את התפריט
}
